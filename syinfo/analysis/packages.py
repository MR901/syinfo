...
class PackageManager:
    ...
    def _detect_available_managers(self) -> List[PackageManagerType]:
        ...
        # Use short per-probe timeout and run in parallel to avoid long waits
        import os
        ...
        def probe(item: tuple[PackageManagerType, List[str]]):
            mgr, cmd = item
            try:
    ...
    def _list_packages_for_manager(
        ...
        if manager == PackageManagerType.APT:
            return self._list_apt_packages(name_filter, installed_only)
        if manager == PackageManagerType.PIP:
            return self._list_pip_packages(name_filter)
        if manager == PackageManagerType.DNF:
            return self._list_dnf_packages(name_filter, installed_only)
        if manager == PackageManagerType.YUM:
            return self._list_yum_packages(name_filter, installed_only)
        if manager == PackageManagerType.CONDA:
            return self._list_conda_packages(name_filter)
        if manager == PackageManagerType.NPM:
            return self._list_npm_packages(name_filter)
        if manager == PackageManagerType.SNAP:
            return self._list_snap_packages(name_filter)
        return []
