...
def create_highlighted_heading(
    ...
    if not isinstance(msg, str):
        raise ValidationError(
            ...
            details={"field_name": "msg", "expected_type": str.__name__},
        )
    if total_length < 20:
        raise ValidationError(
            "Total length must be at least 20", details={"field_name": "total_length"},
        )
...
class Execute:
    ...
    def on_shell(
        ...
        if not cmd or not isinstance(cmd, str):
            raise ValidationError(
                "Command must be a non-empty string", details={"field_name": "cmd"},
            )
        ...
        try:
            ...
            if line_no is not None:
                ...
                if 0 <= line_no < len(lines):
                    ...
                    logger.warning(
                        f"Line number {line_no} out of range for command output",
                    )
                    result = UNKNOWN
            else:
                result = stdout.strip()
            ...
            raise SystemAccessError(f"Command timed out after {timeout} seconds: {cmd}")
        except subprocess.SubprocessError as e:
            raise SystemAccessError(f"Failed to execute command: {cmd} - {e!s}")
        except Exception as e:
    ...
    def api(
        ...
        if not url or not isinstance(url, str):
            raise ValidationError(
                "URL must be a non-empty string", details={"field_name": "url"},
            )

        if not url.startswith(("http://", "https://")):
            raise ValidationError(
                "URL must start with http:// or https://", details={"field_name": "url"},
            )
        ...
        try:
            ...
            request.add_header(
                "User-Agent", "SyInfo/1.0 (+https://github.com/MR901/syinfo)",
            )
            ...
            with urllib.request.urlopen(request, timeout=timeout) as response:
                ...
                if line_no is not None:
                    ...
                    if 0 <= line_no < len(lines):
                        ...
                        logger.warning(
                            f"Line number {line_no} out of range for API response",
                        )
                        result = UNKNOWN
                else:
            ...
            raise SystemAccessError(f"HTTP error {e.code} requesting {url}: {e.reason}")
        except urllib.error.URLError as e:
            raise SystemAccessError(f"URL error requesting {url}: {e.reason}")
        except Exception as e:
            logger.error(f"Unexpected error requesting {url}: {e}")
            raise SystemAccessError(f"Failed to request {url}: {e!s}")
...
def safe_file_read(file_path: Union[str, Path], encoding: str = "utf-8") -> str:
    ...
    if not path.exists():
        raise SystemAccessError(f"File not found: {path}")

    if not path.is_file():
        raise SystemAccessError(f"Path is not a file: {path}")

    try:
        return path.read_text(encoding=encoding)
    except PermissionError:
        raise SystemAccessError(f"Permission denied reading file: {path}")
